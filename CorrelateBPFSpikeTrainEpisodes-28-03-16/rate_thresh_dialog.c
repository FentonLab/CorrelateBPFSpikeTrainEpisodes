...
SESSION	*sess;
{
	...
	while (TRUE) {
		...
		(void) system("clear");
		(void) fprintf(stderr, "\nThe units to be analyzed have the following rate thresholds:\n\n");
		(void) fprintf(stderr, "Cell/Probe\tName\t\tThreshold\n\n");
		
		for (i = 0; i < n_units; ++i) {
			cp = sess->cells[i];
			(void) fprintf(stderr, "%d\t\t%s\t\t%f\n", i, cp->name_str, cp->rate_thresh);
		}
		...
		sess->cells[cell_num]->rate_thresh = thresh;
		sess->secondary_cells[cell_num]->rate_thresh = thresh;
	}

	(void) system("clear");
	(void) fprintf(stderr, "\nThe units will be analyzed with the following rate thresholds:\n\n");
	(void) fprintf(stderr, "Cell/Probe\tName\t\tThreshold\n\n");
